...
## Repo topology
- The SPA and the Ruby backend are **separate git repos at the same folder level**:
  ```
  <root>/
    <backend>/   Ruby API + pass-through host
    spa/         this repo — Vue 3 + Vite, component library, Storybook
  ```
- The frontend workflow operates in `spa/`. When you need the API contract (endpoints,
  payloads, auth), **read the sibling `../<backend>`** (routes/controllers) — do not guess
  the shape. Never reach across for anything but the contract; the SPA talks to the backend
  only over HTTP.
